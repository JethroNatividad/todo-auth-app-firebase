    ...
    <Formik initialValues={{ text: '' }} onSubmit={async ({ text }, { setSubmitting, setValues }) => {
      ...
      {({
        ...
        <form className="flex space-x-1 px-1" onSubmit={handleSubmit}>
          ...
          />
          <button disabled={isSubmitting} className="border-2 rounded-xl border-black px-2 py-1 hover:bg-black hover:text-white transition-all ease-in-out" type="submit">Add</button>
        </form>
      )}
    </Formik>
  );
