...
const Navbar = ({ username, loading, signout }: Props) => {
  return (
    <div className='shadow-sm h-12 flex items-center justify-between px-5'>
      ...
      <button className='px-2 py-1 border-2 border-black rounded-xl hover:bg-black hover:text-white transition-all ease-in-out' onClick={signout} disabled={loading}>{loading ? 'loading' : 'Logout'}</button>
    </div>
  )
}
